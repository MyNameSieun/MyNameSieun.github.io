...
const useTodosStore = create((set) => ({
  ...
  addTodos: async (data) => {
    /* POST 요청 및 새로운 todo 추가 */
  },
  deleteTodos: async (id) => {
    /* DELETE 요청 및 해당 todo 삭제 */
  },
  editTodos: async (id, updatedData) => {
    /* PUT 요청 및 수정된 todo 업데이트 */
  },
  toggleTodo: async (id, isDone) => {
    /* PUT 요청으로 todo의 완료 상태 변경 */
  },
}));
```
...
toggleTodos: async (id) => {
  try {
    ...
    const response = await todoAxios.patch(`/todos/${id}`, {
...
const TodoList = ({ showCompleted }) => {
  const { todos, loading, fetchTodos, deleteTodos, editTodos, toggleTodo } =
    useTodosStore();
