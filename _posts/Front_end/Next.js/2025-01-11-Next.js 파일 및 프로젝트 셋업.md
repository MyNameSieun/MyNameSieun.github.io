...
- 하지만, 아래 코드와 같이 `rafce`로 생성된 기본 템플릿을 그대로 사용할 경우, 페이지의 이름이나 역할을 나타내기 어렵다.
...
소셜 로그인, API URL 등을 관리하려면 `.env`와 `.env.local` 파일을 설정해야한다.
