...
자바 언어로 작성된 프로그램을 컴파일하면 자바 컴파일러는 소스 파일을 플랫폼에 독립적인 코드로 변환한다. 이 플랫폼에 독립적인 코드를 무엇이라고 하는가?
...
main 메소드를 정의할 때, public과 static 중 어떤 수식어가 먼저 나와야 하는가?
