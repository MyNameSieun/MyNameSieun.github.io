...
`visited[start] = true`를 사용하면 자기 자신으로 돌아오는 경로가 누락될 수 있으므로, visited 처리 순서에 유의해야 한다.

> 자기 자신으로 가는 간접 경로 존재 여부

예: 0 → 1 → 2 → 0처럼 순환 구조에서는 간접적으로 자기 자신에 도달 가능함. 이를 누락하지 않도록 visited 초기 처리에 주의해야 함.
